...
# Make happy
.PHONY: bootstrap build-binaries lint-signed lint-code test-unit test-integration test-all build-binaries build-library build lint test clean
...
$(DC_PREFIX)/lib/libmusicoon: $(DC_PREFIX)/bin/tooling/lib/dc-sh-art $(DC_MAKEFILE_DIR)/source/core/*.sh
	$(call title, $@)
...
$(DC_PREFIX)/lib/libmusicoon-strip: $(DC_MAKEFILE_DIR)/source/core/*.sh
	$(call title, $@)
	$(DC_PREFIX)/bin/tooling/bin/dc-tooling-build --destination="$(shell dirname $@)" --name="$(shell basename $@)" --license="MIT License" --author="dubo-dubon-duponey" --description="the library version" --with-git-info $^

# Builds all the CLIs that depend just on the main library
$(DC_PREFIX)/bin/%: $(DC_PREFIX)/lib/libmusicoon $(DC_MAKEFILE_DIR)/source/cli/%
	$(call title, $@)
...
$(DC_PREFIX)/bin/%: $(DC_PREFIX)/lib/libmusicoon $(DC_PREFIX)/bin/tooling/lib/dc-sh-art-extensions $(DC_MAKEFILE_DIR)/source/cli-ext/%
	$(call title, $@)
	$(DC_PREFIX)/bin/tooling/bin/dc-tooling-build --destination="$(shell dirname $@)" --name="$(shell basename $@)" --license="MIT License" --author="dubo-dubon-duponey" --description="another fancy piece of shcript" $^
...
build-library: bootstrap $(DC_PREFIX)/lib/libmusicoon # $(DC_PREFIX)/lib/libmusicoon-strip
