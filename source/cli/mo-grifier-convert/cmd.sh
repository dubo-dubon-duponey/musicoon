...
case "$(printf "%s" "$codec" | tr '[:upper:]' '[:lower:]')" in
  ...
  flac)
    ...
    codec="flac -compression_level 8" # XXX too slow --exhaustive-model-search"
    finalextension=flac
  ;;
esac
...
# Go for it
dc::logger::info "ffmpeg ${ar[*]}"

# Damn you ffmpeg: https://mywiki.wooledge.org/BashFAQ/089
if ! ffmpeg "${ar[@]}" </dev/null; then
...
printf "%s\n" "$destination/$filename.$finalextension"
